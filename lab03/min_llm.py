...
class MiniGPT(nn.Module):
    def __init__(self, vocab_size, n_embd=128, n_head=4,
                 n_layer=2, block_size=128, use_pos=True):
        super().__init__()
        ...
        self.head = nn.Linear(n_embd, vocab_size, bias=False)                 # 权重共享
        self.head.weight = self.tok_emb.weight
        self.apply(self._init_weights)                                        # 小方差初始化

    def _init_weights(self, m):
        """GPT 标准初始化：权重 N(0, 0.02)，偏置清零 -> 初始 loss 约等于 ln V"""
...
def main():
    ...
    ap.add_argument("--no_pos", action="store_true", help="去掉位置编码（对照实验 exp5）")
    ...
    ap.add_argument("--device", type=str, default="cpu", choices=["cpu", "cuda", "auto"],
                    help="训练设备；指南基线为 cpu")
    ap.add_argument("--prompts", type=str, default="春,月",
                    help="生成用的提示词，逗号分隔")
    ap.add_argument("--n_samples", type=int, default=2, help="每个提示词生成几段")
    ap.add_argument("--tag", type=str, default=None,
                    help="输出文件标签，默认按配置自动生成")
    ap.add_argument("--save_model", type=str, default=None,
                    help="训练后保存模型权重（.pt），供采样实验复用")
    ap.add_argument("--load_model", type=str, default=None,
                    help="载入已有权重，跳过训练（配合 --iters 0）")
    args = ap.parse_args()
    ...
    if device == "cuda" and not torch.cuda.is_available():
        raise SystemExit("指定了 cuda，但 torch.cuda.is_available() 为 False")
    if device == "cuda":
        torch.cuda.manual_seed_all(args.seed)
        cap = torch.cuda.get_device_capability(0)
        print(f"PyTorch {torch.__version__} | 设备: {device} | "
              f"GPU: {torch.cuda.get_device_name(0)} | capability sm_{cap[0]}{cap[1]}")
    else:
        print(f"PyTorch {torch.__version__} | 设备: {device} | "
    ...
    # 模型
    model = MiniGPT(tok.vocab_size, args.n_embd, args.n_head,
                    args.n_layer, args.block_size, use_pos=not args.no_pos).to(device)
    n_params = sum(p.numel() for p in model.parameters())
    posflag = "有" if not args.no_pos else "无"
    print(f"模型参数量: {n_params:,} | 配置: n_layer={args.n_layer} n_head={args.n_head} "
          f"n_embd={args.n_embd} block={args.block_size} pos={posflag}")
    ...
    if args.load_model:
        ...
        model.eval()
        print(f"已载入权重 {args.load_model}，跳过训练")
        ...
        for step in range(1, args.iters + 1):
            ...
            if step % 100 == 0 or step == 1:
                ...
                print(f"step {step:5d}/{args.iters} | loss {loss.item():.4f} | "
                      f"{speed:.2f} it/s | 预计剩余 {eta:.0f}s", flush=True)
        train_time = time.time() - t0
        print(f"训练完成，耗时 {train_time:.1f} 秒 = {train_time/60:.2f} 分钟 | "
              f"初始 loss {losses[0]:.4f} | 最终 loss {losses[-1]:.4f} | "
              f"前 100 步均值 {sum(losses[:100])/len(losses[:100]):.4f}")
        if args.save_model:
            torch.save(model.state_dict(), args.save_model)
            print(f"已保存权重 {args.save_model}")

    tag = args.tag or ("no_pos" if args.no_pos
                       else f"L{args.n_layer}_E{args.n_embd}_lr{args.lr:g}")
    ...
    # 生成
    model.eval()
    samplefile = f"generated_{tag}.txt"
    chunks = []
    for prompt in [p for p in args.prompts.split(",") if p]:
        for k in range(args.n_samples):
            out = model.generate(torch.tensor([tok.encode(prompt)], device=device),
                                 args.max_new_tokens, args.temperature, args.top_k)
            sample = tok.decode(out[0].tolist())
            print("=" * 64)
            print(f"生成示例「{prompt}」#{k+1}  (temperature={args.temperature}, "
                  f"top_k={args.top_k})")
            print(sample)
            chunks.append(f"# 提示词「{prompt}」 #{k+1}  "
                          f"temperature={args.temperature}  top_k={args.top_k}\n{sample}\n")
    with open(samplefile, "w", encoding="utf-8") as f:
        f.write("\n".join(chunks))
    print(f"已保存 {samplefile}")
